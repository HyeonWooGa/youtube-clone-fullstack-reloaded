...
export const finishGithubLogin = async (req, res) => {
    ...
    if("access_token" in tokenRequest) {
        ...
        let user = await User.findOne({emil: emailObj.email});
        if(!user) {
            await User.create({
...
export const postEdit = async (req, res) => {
    const { session: {user : {_id, avatarUrl}}, body: { name, email, username, location}, file} = req; // ES6 const id = req.session.user.id & const { name, email, username, location } = req.body;
...
export const postChangePassword = async (req, res) => {
    ...
    if(!ok) {
        return res.status(400).render("users/change-password", { pageTitle: 
            "Change Password", errorMessage: "The current password is incorrect",});
    }
    if(newPassword !== newPasswordConfirmation) {
        return res.status(400).render("users/change-password", { pageTitle: 
            "Change Password", errorMessage: "The password does not match the confirmation",});
    }
